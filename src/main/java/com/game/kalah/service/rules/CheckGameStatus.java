...
/**
 * This class verifies the final status of the game, sets final pits position
 * and winner
 * 
...
public class CheckGameStatus implements KalahRule {
	...
	/**
	 * This method verifies the final status of the game based on sum of stones in
	 * each players pits. If sum of stones in any player's pits is zero, then game
	 * is finished. Decides the winner based on final scores and set final pit
	 * positions.
	 * 
	...
	/**
	 * This method calculates total score i.e. sum of stones in the pits + stones in
	 * home pit
	 * 
	 * @param kalah  {@link Kalah}
	...
	/**
	 * This method set the final position of the pits i.e. when game is finished set
	 * the home pits with total scores and rest of the pits with 0.
	 * 
	 * 
	 * @param kalah                  {@link Kalah}
	 ...
	 */
	private void setFinalPitPositions(Kalah kalah, int sumOfStonesNorthPlayer, int sumOfStonesSouthPlayer) {
	...
	private int getPitsSum(int[] pits, Player player) {
		...
		return IntStream.range(0, pits.length)
				.filter(pitId -> pitId >= player.getPitStartPosition() && pitId <= player.getPitEndPosition())
				.map(pitId -> pits[pitId]).sum();
	}

	/**
	 * This method checks if the sum of all the pits is 0 for any player set game
	 * status to FINISHED
	 * 
	 * @param kalah {@link Kalah}
	 */
